...
このドキュメントは，コードクローン追跡システム ECTEC の使用方法並びに内部で行われている処理内容について述べているドキュメントである．
ECTEC は Java で記述されており，内部でリレーショナルデータベースを用いている．
...
\item[対象バージョン管理システム: ] Subversion

\end{description}

以降の本ドキュメントの構成は次の通り．
はじめに\ref{sec:quick-start}で ECTEC の簡単な使用方法について述べる．
次に\ref{sec:processing}において内部で行われている処理内容について述べ，
\ref{sec:data-structure}でデータ構造について説明する．
最後に\ref{sec:how-to-use}において ECTEC の使用方法について詳細に述べる．
%最後に\ref{sec:project}で ECTEC のプロジェクト構成について言及する．
